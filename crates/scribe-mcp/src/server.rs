...
use scribe_core::store::{open_scribe_store, ScribeStore};
...
impl ScribeMcp {
    ...
    #[tool(description = "Full-text search across Scribe notes (titles + body). Use first when recalling facts.")]
    fn search_documents(
        &self,
        ...
        self.with_store(|store| {
            let hits = store.search_documents(&params.query, params.limit.unwrap_or(10))?;
            Ok(tools::json(&serde_json::json!({
                "query": params.query,
                "count": hits.len(),
                "hits": hits,
